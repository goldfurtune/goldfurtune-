...
async function loadData(){try{const r=await fetch(BIN_URL+'/latest',{headers:{'X-Master-Key':MASTER_KEY,'X-Access-Key':ACCESS_KEY},cache:'no-store'});if(!r.ok)throw 0;const j=await r.json();const rec=j.record||j;appData={users:rec.users||[],withdrawals:rec.withdrawals||[],settings:rec.settings||{whatsapp:'https://wa.me/2348148103620',telegram:'https://t.me/goldfortune01',telegramChannel:'https://t.me/goldfortune01',bankAccount:'1848130535',bankName:'Smartcash',accountName:'Blessing Asuquo',adminPassword:'202020'},products:rec.products||PRODUCTS};localStorage.setItem('gf_data',JSON.stringify(appData));}catch(e){const l=localStorage.getItem('gf_data');if(l)try{appData=JSON.parse(l);}catch{}}}
async function saveData(){localStorage.setItem('gf_data',JSON.stringify(appData));try{const r=await fetch(BIN_URL,{method:'PUT',headers:{'Content-Type':'application/json','X-Master-Key':MASTER_KEY,'X-Access-Key':ACCESS_KEY},body:JSON.stringify(appData)});return r.ok;}catch{return false;}}
function getSession(){const s=localStorage.getItem('gf_session');return s?JSON.parse(s):null;}
function setSession(u){localStorage.setItem('gf_session',JSON.stringify({id:u.id,email:u.email}));currentUser=u;}
function clearSession(){localStorage.removeItem('gf_session');currentUser=null;}
async function handleSignup(){const name=document.getElementById('signup-name').value.trim();const email=document.getElementById('signup-email').value.trim().toLowerCase();const pass=document.getElementById('signup-password').value;const conf=document.getElementById('signup-confirm').value;const ref=document.getElementById('signup-ref').value.trim().toUpperCase();if(!name||!email||!pass)return showToast('Fill all');if(pass.length<6)return showToast('Min 6');if(pass!==conf)return showToast('No match');await loadData();if(appData.users.find(u=>u.email===email))return showToast('Email exists');const user={id:generateId(),fullName:name,email,password:pass,referralCode:generateRefCode(),referredBy:ref||null,balance:5000,totalEarned:5000,referrals:[],bank:null,purchases:[],transactions:[{type:'bonus',amount:5000,note:'Welcome bonus',date:new Date().toISOString()}],createdAt:new Date().toISOString()};if(ref){const ru=appData.users.find(u=>u.referralCode===ref);if(ru){ru.balance=(ru.balance||0)+2000;ru.totalEarned=(ru.totalEarned||0)+2000;ru.referrals=ru.referrals||[];ru.referrals.push({id:user.id,name:user.fullName,email:user.email,date:user.createdAt});ru.transactions=ru.transactions||[];ru.transactions.unshift({type:'referral',amount:2000,note:`Referral: ${user.fullName}`,date:new Date().toISOString()});}}appData.users.push(user);await saveData();setSession(user);updateSupportLinks();showAuthSpinner(()=>{showPage('welcome');triggerWelcomeBalloons();});}
async function handleLogin(){const email=document.getElementById('login-email').value.trim().toLowerCase();const pass=document.getElementById('login-password').value;if(!email||!pass)return showToast('Enter email and pass');await loadData();const user=appData.users.find(u=>u.email===email&&u.password===pass);if(!user)return showToast('Invalid');setSession(user);updateSupportLinks();resetIdleTimer();showAuthSpinner(()=>{showPage('dashboard');startLiveActivity();resetIdleTimer();});}
function handleForgot(){showToast('Reset link sent');setTimeout(()=>showPage('login'),2000);}
function logout(){stopLiveActivity();stopSyncLoop();if(idleTimer)clearTimeout(idleTimer);clearSession();showPage('login');}
function showPage(p){document.querySelectorAll('[id^="page-"]').forEach(e=>e.classList.add('hidden'));document.getElementById('app-shell')?.classList.add('hidden');if(['login','signup','forgot','welcome'].includes(p)){document.getElementById('page-'+p)?.classList.remove('hidden');if(p==='signup'){const params=new URLSearchParams(location.search);const ref=params.get('ref');if(ref)document.getElementById('signup-ref').value=ref;}if(p==='welcome'){const tg=appData.settings?.telegram||'https://t.me/goldfortune01';const el=document.getElementById('welcome-telegram');if(el)el.href=tg;triggerWelcomeBalloons();}return;}if(!currentUser){showPage('login');return;}document.getElementById('app-shell').classList.remove('hidden');document.getElementById('page-'+p)?.classList.remove('hidden');if(p==='dashboard')renderDashboard();if(p==='products')renderProducts();if(p==='withdraw')renderWithdraw();if(p==='invite')renderInvite();if(p==='profile')renderProfile();if(p==='support'||p==='platform')updateSupportLinks();if(p==='add-bank')populateBanks();if(p==='dashboard'&&currentUser)startLiveActivity();else stopLiveActivity();resetIdleTimer();}
