...
const PAYSTACK_SECRET = 'sk_test_52822313ae583b3056e9aeb5e385c7eb84533606';

let paystackBanks = [];      // [{ name, code }]
let filteredBanks = [];
...
function setConfirmButtonState() {
  ...
  if (!btn) return;
  const ready = accountVerified && selectedBankCode && (document.getElementById('bank-acc')?.value || '').replace(/\D/g, '').length === 10;
  btn.disabled = !ready;
  if (ready) {
    btn.classList.remove('opacity-50', 'cursor-not-allowed');
    btn.textContent = 'Confirm & Add Bank Account';
  } else {
    btn.classList.add('opacity-50', 'cursor-not-allowed');
    btn.textContent = accountVerified ? 'Confirm & Add Bank Account' : 'Verify Account First';
  }
}
...
async function loadPaystackBanks() {
  ...
  if (list) {
    list.innerHTML = '<p class="p-4 text-sm text-[#a08c6a] text-center">Loading banks...</p>';
  }
  try {
    ...
    if (!res.ok || !data.status) throw new Error(data.message || 'Failed to load banks');
    paystackBanks = (data.data || []).map(b => ({ name: b.name, code: b.code })).sort((a, b) => a.name.localeCompare(b.name));
    filteredBanks = [...paystackBanks];
    renderBankList();
  } catch (err) {
    if (list) {
      list.innerHTML = `<p class="p-4 text-sm text-center" style="color:#f87171">Could not load banks: ${err.message}. Try again.</p>
        <button type="button" onclick="loadPaystackBanks()" class="btn-outline mx-4 mb-4">Retry Load Banks</button>`;
    }
...
function filterBanks() {
  ...
  filteredBanks = q
    ? paystackBanks.filter(b => b.name.toLowerCase().includes(q))
    : [...paystackBanks];
  renderBankList();
}
...
function renderBankList() {
  ...
  list.innerHTML = filteredBanks.map(b => {
    const checked = selectedBankCode === b.code ? 'checked' : '';
    const active = selectedBankCode === b.code ? 'style="background:#2a2215"' : '';
    const safeName = b.name.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;');
    return `<label class="flex items-center justify-between px-4 py-3.5 border-b border-[#2e2618] cursor-pointer hover:bg-[#221c12]" ${active} data-code="${b.code}" data-name="${safeName}">
      <span class="text-sm">${safeName}</span>
      <input type="radio" name="bank-radio" class="accent-[#d4a536] w-4 h-4" ${checked} />
    </label>`;
  }).join('');
  list.querySelectorAll('label[data-code]').forEach(el => {
    el.addEventListener('click', () => selectBank(el.getAttribute('data-code'), el.getAttribute('data-name')));
  });
}
...
function onAccountNumberInput() {
  ...
  resolveTimer = setTimeout(async () => {
    try {
      const url = `https://api.paystack.co/bank/resolve?account_number=${encodeURIComponent(acc)}&bank_code=${encodeURIComponent(selectedBankCode)}`;
      const res = await fetch(url, {
        headers: { 'Authorization': 'Bearer ' + PAYSTACK_SECRET }
      });
      const data = await res.json();
      if (!res.ok || !data.status) {
        throw new Error(data.message || 'Could not resolve this account.');
      }
      const resolved = (data.data.account_name || '').toUpperCase();
      if (nameInput) nameInput.value = resolved;
      if (spin) spin.classList.add('hidden');
      if (msg) {
        msg.textContent = '✓ Account name verified';
      ...
      if (spin) spin.classList.add('hidden');
      if (msg) {
        msg.textContent = err.message || 'Could not resolve account';
        msg.style.color = '#f87171';
        msg.classList.remove('hidden');
      }
      accountVerified = false;
...
async function saveBank() {
  if (!accountVerified) return showToast('Please verify the account first');
  ...
  if (!bankName || !selectedBankCode) return showToast('Select a bank');
  if (!accountNumber || accountNumber.length < 10) return showToast('Enter valid 10-digit account number');
  if (!accountName) return showToast('Account name not resolved');

  currentUser.bank = { bankName, accountNumber, accountName, bankCode: selectedBankCode };
  const idx = appData.users.findIndex(u => u.id === currentUser.id);
  if (idx >= 0) appData.users[idx] = currentUser;
  await saveData();
