...
async function loadData(){
  try{
    // FIX: Try ACCESS_KEY first (valid 60 chars) - your MASTER_KEY was truncated 53 chars causing Offline
    ...
    appData={users:rec.users||[],withdrawals:rec.withdrawals||[],settings:rec.settings||{whatsapp:'https://wa.me/2348148103620',telegram:'https://t.me/goldfortune01',telegramChannel:'https://t.me/goldfortune01',bankAccount:'1848130535',bankName:'Smartcash',accountName:'Blessing Asuquo',adminPassword:'202020'},products:rec.products||PRODUCTS};
    localStorage.setItem('gf_data',JSON.stringify(appData));
  }catch(e){
    console.log('loadData offline, using cache',e);
...
async function saveData(){
  localStorage.setItem('gf_data',JSON.stringify(appData));
  try{
    // Try with both keys first
    let r = await fetch(BIN_URL,{method:'PUT',headers:{'Content-Type':'application/json','X-Master-Key':MASTER_KEY,'X-Access-Key':ACCESS_KEY},body:JSON.stringify(appData)});
    if(r.ok) return true;
    // Fallback try Access Key only (if Access Key has write)
    r = await fetch(BIN_URL,{method:'PUT',headers:{'Content-Type':'application/json','X-Access-Key':ACCESS_KEY},body:JSON.stringify(appData)});
    return r.ok;
  }catch(e){console.log('saveData offline',e);return false;}
}
